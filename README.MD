...
The repository now contains a deterministic Eclipse Temurin Java 21 JRE manifest, static validation and normalized bundle packaging, the secure Change 003 installer, and a bounded production automatic-runtime relaunch integration for Linux glibc, macOS, and Windows on `x86_64` and `aarch64`. The normalized runtime bundle remains a separate client payload at `lwjgl3ify/runtime/lwjgl3ify-wdg-java21-runtimes.zip`; it is never embedded in the mod JAR or placed in `mods/`. See [docs/BUNDLED_JAVA.md](docs/BUNDLED_JAVA.md). Distant Horizons integration remains deferred.
