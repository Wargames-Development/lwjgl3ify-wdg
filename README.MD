...
The repository now contains a deterministic Eclipse Temurin Java 21 JRE manifest, static validation and normalized bundle-packaging tasks, and a secure explicit-input extraction/cache installer for Linux, macOS, and Windows on `x86_64` and `aarch64`. See [docs/BUNDLED_JAVA.md](docs/BUNDLED_JAVA.md). Production startup does not invoke the installer yet: packaged-bundle discovery, host selection, Java-list/config changes, automatic relaunch, and DistantHorizons-WDG integration remain deferred; existing production relauncher behavior is unchanged.
